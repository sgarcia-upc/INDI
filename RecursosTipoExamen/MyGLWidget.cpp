...
void MyGLWidget::iniEscena ()
{
    ...
    //std::cout << centrePatr << std::endl;
    //std::cout << escala.x << std::endl;
    //std::cout << escala.y << std::endl;
    //std::cout << escala.z << std::endl;
    

    centreEsc = (max + min)/ glm::vec3(2);
...
void MyGLWidget::modelTransformPatricioInverso ()
{
    glm::mat4 TG(1.f);  // Matriu de transformació
    TG = glm::translate(TG, glm::vec3(0,2,0));
    TG = glm::rotate(TG, glm::radians(180.0f), glm::vec3(0,0,1));
    TG = glm::scale(TG, glm::vec3(escala, escala, escala));
    TG = glm::translate(TG, -centrePatr);
...
void MyGLWidget::viewTransform ()
{
    ...
    View = glm::rotate(View, theta, glm::vec3(1, 0.0, 0.0));
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if (DoingInteractiveRight == ROTATE)
    {
        // Fem la rotació
        //theta += (e->y() - yClick) * M_PI / 180.0;
        zoom += (e->y() - yClick);
        fov = M_PI/(zoom*0.9);
        std::cout << M_PI/zoom << std::endl;
...
void MyGLWidget::calculaCapsaModel ()
{
    ...
    escala = 2.0/(maxy-miny); // 0.322678
    centrePatr[0] = (minx+maxx)/2.0; centrePatr[1] = (miny+maxy)/2.0; centrePatr[2] = (minz+maxz)/2.0;

    min = glm::vec3(-2.0, -1, -2.0);
    max = glm::vec3(2.0, (maxy-miny)/2, 2.0);
    std::cout << (maxy-miny) << std::endl; // 6.19812
    std::cout << 2/(maxy-miny) << std::endl; // 0.322678
    std::cout << (maxy-miny)/2 << std::endl; // 3.09906
}
